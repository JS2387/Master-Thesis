...
result <- data.frame(Model = 'Simple LM-Bowlers: Without Intercept',
...
#restricted LM
anova(lm.model)
lm.model <- lm(Price ~ Overseas_player + Star_player + Active_T20I + Cum_LY_Bowl_I + Cum_IPL_bowl_DM_Bowl_I
               , data = train)
summary(lm.model)
predictions <- predict(lm.model, newdata = test)
...
glm.model <- glm(Price ~ Overseas_player + Star_player + Active_T20I + Cum_LY_Bowl_I + Cum_IPL_bowl_DM_Bowl_I,
...
result <- data.frame(Model = 'Simple GLM-Bowlers:Gaussian family log link',
                     RMSE = rmse(test$Price, predictions),
                     MAPE = mape(test$Price, predictions),
                     AIC = AIC(glm.model),
                     BIC = BIC(glm.model),
...
#LMM-Restricted
Anova(lmm.model)
lmm.model <- lmer(Price ~ Season_num + Star_player + Active_T20I + Cum_LY_Bowl_I +  Cum_IPL_bowl_DM_Bowl_I +
...
result <- data.frame(Model = 'GLMM-Bowl: Gaussian Family: RE Name',
                     RMSE = rmse(test$Price, predictions),
                     MAPE = mape(test$Price, predictions),
                     AIC = AIC(glmm_model),
                     BIC = BIC(glmm_model),
...
glmm_model <- glmer(Price ~ Season_num + Star_player + Active_T20I + Cum_LY_Bowl_I +  Cum_IPL_bowl_DM_Bowl_I +
                      (1 | Name2), 
                    data = train,
                    family = "gaussian")
...
result <- data.frame(Model = 'Restricted GLMM-Bowl: Gaussian Family: RE Name',
                     RMSE = rmse(test$Price, predictions),
                     MAPE = mape(test$Price, predictions),
                     AIC = AIC(glmm_model),
                     BIC = BIC(glmm_model),
...
result <- data.frame(Model = 'Restricted GLMM-Bowl: Gaussian Family: RE Cum_LY_Bowl_I  + Name',
                     RMSE = rmse(test$Price, predictions),
                     MAPE = mape(test$Price, predictions),
                     AIC = AIC(glmm_model),
                     BIC = BIC(glmm_model),
...
#prior<-list(R=list(V=1, nu=0.002), G=list(G1=list(V=1, nu=0.002)))
MCMCglmm_model <- MCMCglmm(fixed = Price ~ Season_num + Star_player + Cum_LY_Bowl_I + Cum_LY_bowl_46_conc + Cum_IPL_bowl_DM_Bowl_I,
                           random = ~ Name2,
                           data = train,
                           prior = prior,
                           family = "exponential",
...
result <- data.frame(Model = 'Restricted MCMC GLMM-Bowl: RE Name',
                     RMSE = rmse(test$Price, predictions),
                     MAPE = mape(test$Price, predictions),
                     AIC = NA,
                     BIC = NA,
...
write.csv(results_df, "Bowlers modelling results2.csv")
